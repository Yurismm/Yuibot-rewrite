...
async def osustats(ctx, *, osuplayer: str = None):
    if osuplayer == None:
        ...
        embed.set_image(
            url=
            f"http://lemmmy.pw/osusig/sig.php?colour=hexff66aa&uname={osuplayer}&pp=1&countryrank&flagshadow&flagstroke&opaqueavatar&avatarrounding=5&onlineindicator=undefined&xpbar&xpbarhex&random={time.time()}"
        )
        await ctx.send(embed=embed)
